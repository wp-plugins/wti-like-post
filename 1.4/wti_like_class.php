...
/*class RecentlyLikedPosts
{
     ...
     function widget($args, $instance = array() ){
		...
		//getting the most liked posts
		$query = "SELECT post_id, SUM(value) AS like_count, post_title, user_id FROM `{$wpdb->prefix}wti_like_post` L JOIN {$wpdb->prefix}posts P ";
		$query .= "ON L.post_id = P.ID WHERE post_status = 'publish' AND value > 0 AND user_id > 0 $where GROUP BY post_id ORDER BY date_time DESC LIMIT $number";
		...
		if(count($posts) > 0) {
			foreach ($posts as $post) {
				...
				$nice_name = $user_data->user_nicename;
				
				$widget_data .= '<li>' . $nice_name . ' ' . __('likes', 'wti-like-post') . ' <a href="' . $permalink . '" title="' . $post_title.'" rel="nofollow">' . $post_title . '</a>';
				$widget_data .= '</li>';
			}
		} else {
			$widget_data .= '<li>';
